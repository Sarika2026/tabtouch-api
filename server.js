...
function processRace(data, raceId) {
    ...
    let rows = horses.map((h, i) => {
        ...
        return { A, D, J: h.number, K, L: h.jockey, N: h.barrier, H, P, color, Z:"", Zcolor:0 }
    });
...
app.post("/api/tabtouch", async (req, res) => {
    try{
        ...
        const runners = tabData.runners.map(r => ({number:r.number,name:r.name,odds:parseFloat(r.fixedOddsWin)||parseFloat(r.toteWin)||0,jockey:r.jockey,barrier:r.barrier}));
